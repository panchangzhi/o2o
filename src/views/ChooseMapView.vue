        ...
        created(){
            ...
            storage.getItem('nowLocation',ele => {
                if(ele.data != "undefined"){
                    ...
                    _this.longitude = data.lng || data.longitude ;
                    _this.latitude = data.lat || data.latitude;
                }
            })
        },
        methods:{
            submit(){
                if(this.$route.params.type == 'checkIn')//商家入驻获取地理位置
                {
                    ...
                    storage.getItem('editData',res=>{
                        store = res.data != 'undefined' ? JSON.parse(res.data) : {};
                        store.address = this.locationText;
                        store.Lng = this.longitude;
                        store.Lat = this.latitude;
                        storage.setItem('editData',JSON.stringify(store));
                    });
                }
